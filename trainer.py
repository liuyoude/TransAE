...
class Trainer:
    def __init__(self, *args, **kwargs):
        ...
        self.criterion = ASDLoss(w_mse=self.args.w_mse, w_ce=self.args.w_ce).to(self.args.device)
    ...
    def train(self, train_loader, valid_dir):
        ...
        for epoch in range(0, epochs + 1):
            ...
            # if (epochs + 1) % valid_every_epochs == 0:
            #     self.criterion.w_mse = 0
            #     self.criterion.w_ce = 1
            # else:
            #     self.criterion.w_mse = 1
            #     self.criterion.w_ce = 0
            for (x_mels, x_ps, labels) in train_bar:
                # forward
                b, n, d = x_mels.shape
                n_fft = x_ps.shape[2] // frames
                inputs = x_mels.reshape(b*n, d).float()
                phases = x_ps.reshape(b*n, d).float()
                targets = inputs
                if self.args.idnn:
                    ...
                    cf_mask = torch.ones_like(x_ps)
                    cf_mask[:, :, n_fft * (frames // 2): n_fft * (frames // 2 + 1)] = 0
                    phases = phases[cf_mask.bool()].reshape(b*n, -1)

                    frames -= 1
                inputs = inputs.reshape(b*n, frames, -1).to(self.args.device)
                phases = phases.reshape(b*n, frames, -1).to(self.args.device)
                targets = targets.reshape(b*n, frames, -1).to(self.args.device)
                labels = labels.reshape(-1).to(self.args.device)
                outputs, logits = self.net(inputs, phases)
                loss, mse_loss, ce_loss = self.criterion(outputs, logits, targets, labels)
                ...
                self.optimizer.step()
                # visualization
                self.writer.add_scalar(f'{self.args.machine}/loss', loss.item(), self.sum_train_steps)
                self.writer.add_scalar(f'{self.args.machine}/mse_loss', mse_loss.item(), self.sum_train_steps)
                self.writer.add_scalar(f'{self.args.machine}/ce_loss', ce_loss.item(), self.sum_train_steps)
                sum_loss += loss.item()
    ...
    def valid(self, valid_dir, save=True, result_dir=None, csv_lines=[]):
        ...
        for id_str in machine_id_list:
            ...
            for file_idx, file_path in enumerate(test_files):
                x_mels, phases, labels = self.transform(file_path)
                n, d = x_mels.shape
                n_fft = phases.shape[2] // frames
                inputs = x_mels.float()
                targets = inputs
                if self.args.idnn:
                    cf_mask = torch.ones_like(x_mels)
                    ...
                    cf_mask = torch.ones_like(phases)
                    cf_mask[:, :, n_fft * (frames // 2): n_fft * (frames // 2 + 1)] = 0
                    phases = phases[cf_mask.bool()].reshape(n, -1)

                    frames -= 1
                inputs = inputs.reshape(n, frames, -1).to(self.args.device)
                phases = phases.reshape(n, frames, -1).to(self.args.device)
                targets = targets.reshape(n, frames, -1).to(self.args.device)
                labels = labels.reshape(-1).to(self.args.device)
                with torch.no_grad():
                    outputs, _ = net(inputs, phases)
