...
class Wave2Mel(object):
    def __init__(self, sr,
        ...
        self.spec_transform = torchaudio.transforms.Spectrogram(n_fft=n_fft,
    ...
    def __call__(self, x):
        ...
        return self.amplitude_to_db(self.mel_transform(x)), torch.angle(self.spec_transform(x))


def get_id_label_dict(dirs: list):
    id_list = []
    for dir in dirs:
        id_list.extend(get_machine_id_list(dir))
    id_list = sorted(id_list)
    id2label, label2id = {}, {}
    for label, id in enumerate(id_list):
        id2label[id] = label
        label2id[label] = id
